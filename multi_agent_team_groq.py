...
║   4. EmailAgent        — Drafts and optionally sends emails                  ║
║                                                                              ║
║  Requirements:                                                               ║
...
# ── Email settings (optional — for real sending via SMTP) ──
...
class ResearchAgent(BaseAgent):
    ...
    Uses Groq's fast LLaMA model for quick retrieval-style responses.
    """
...
class SummarizerAgent(BaseAgent):
    ...
    Uses Groq's instant model for speed since summarization is simpler.
    """
...
class EmailAgent(BaseAgent):
    """
    Drafts professional emails and optionally sends them via SMTP.
    """
...
Keep the tone warm, clear, professional, and action-oriented.
    ...
    def run(
        ...
        if SMTP_USER and SMTP_PASSWORD:
            ...
            self._log("SMTP not configured — email drafted only (not sent).")

        return drafted_email

    def _send_email(self, drafted_email: str, recipients: list[str]):
        """Send via Microsoft 365 / Gmail SMTP."""
        import smtplib
        ...
        subject = next(
            (l.replace("Subject:", "").strip() for l in lines if l.lower().startswith("subject:")),
            "AI Research Update",
        )
        body_start = next(
        ...
        try:
            ...
            self._log(f"Email sent to {recipients} ✓")
        except Exception as exc:
            self._log(f"SMTP send failed: {exc}")
